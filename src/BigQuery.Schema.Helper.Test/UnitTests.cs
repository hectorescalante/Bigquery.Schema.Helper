...
namespace BigQuery.Schema.Helper.Test
{
  public class UnitTests
  {
    ...
    public void TestGetRowFromEntity_WithInstance_ShouldReturnBigQueryInsertRow()
    {
      ...
      Assert.Equal(timestamp.ToString("yyyy-MM-dd HH:mm:ss"), row["CreatedAt"]);
      ...
      Assert.Collection((List<string>)row["ModifiedAt"],
        item1 => { Assert.Equal(timestamp.AddDays(-1).ToString("yyyy-MM-dd HH:mm:ss"), item1); },
        item2 => { Assert.Equal(timestamp.AddDays(-2).ToString("yyyy-MM-dd HH:mm:ss"), item2); }
      );
      ...
      Assert.Equal(timestamp.ToString("yyyy-MM-dd HH:mm:ss"), ((BigQueryInsertRow)row["Detail"])["Timestamp"]);
      ...
      Assert.Collection((List<BigQueryInsertRow>)row["History"],
        ...
        item5 =>
        {
          ...
          Assert.Equal(timestamp.ToString("yyyy-MM-dd HH:mm:ss"), item5["Timestamp"]);
        }
      );
